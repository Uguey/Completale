...
$(document).ready(function(){
	...
	$("#addUserTwo").click(function(event){
		event.preventDefault();
		var value = $("#inputParticipantsStoryTwo").val();
		participantsTwo[j] = value.toString();
		$("#adjustElementsInsideTwo").after("<input type=\"hidden\" name=" + "\"" + j + "\""+ "value=" +  "\"" + participantsTwo[j] + "\"" +">");		
		if (value!="") {
			$("#divNamesTwo").html(function(i,origText){
	...
	$("#inputParticipantsStory").keyup(function(){retrieveSearchFollowers(participants, "")});
	$("#inputParticipantsStoryTwo").keyup(function(){retrieveSearchFollowers(participants, "Two")});

	window.setInterval(updateData, 1000);
	var listenerTitle = window.setInterval(function(){addListener(listenerTitle)}, 1000);
	
	$("#storiesEraseTwo").click(function(){eraseStoriesTwo()});
	$("#storiesErase").click(function(){eraseStories()});

});

	function addListener (listenerTitle){
		var challenge = document.getElementsByClassName("buttonTimelineChallenge");
		if (challenge.length>0) {
			$(".buttonTimelineChallenge").click(function(){
				var titleText = this.previousSibling.firstChild.nodeValue;
				$("#addChallenge").html("<input type=\"hidden\" name=\"title\" value=\"" + titleText + "\">");
			});
			window.clearInterval(listenerTitle);
		}		
	}			
					...
					challengesTimeline +=
						"<div class=\"well well-xs adjustTimeline\">" + 
							"<div class=\"sameRow\">" +
								...
								"<button class=\"btn btn-primary btn-md buttonTimeline buttonTimelineChallenge\"" +
								 "data-toggle=\"modal\" data-target=\"#storyForChallenge\">Participar</button>" +
							"</div>" +
	...
	function retrieveSearchFollowers (chosen, two) {
		var searched = $("#inputParticipantsStory" + two).val();
		var chosenString = chosen.join(", ");
		$.post("/completaleWEB/RetrieveSearchFollowers",
		{
			search: searched,
			alreadyChosen: chosenString
		},
		function(data, status){
			if (status=="success"){
				...
				for (var i=0;i<newData.length;i++){
					searches += 
					"<option value="+ newData[i] + ">" + newData[i] + "</option>";
				}
				$("#participantsStory" + two).html(searches);
			}
			else console.log("Error al obtener los datos de la búsqueda mediante AJAX");
		});
	}
	
	function eraseStoriesTwo () {

		for (z=0;z<=j;z++){
			$("input[name=\"" + z.toString() + "\"]").remove();
		}
